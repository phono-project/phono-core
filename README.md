...
如需模拟实时输入法编辑，可运行 `results/ime_demo_capi phonop2c_v2_0_base_model`。程序支持左右移动光标以及 Backspace/Delete 删除；第 0 栏实时显示自动切分结果，第 1-N 栏显示候选，按 Ctrl-C 立即退出。
...
For real-time editing with left/right movement and backspace/delete, run `results/ime_demo_capi phonop2c_v2_0_base_model`. Bar 0 shows automatic segmentation and bars 1-N show candidates after every edit; press Ctrl-C to exit.
