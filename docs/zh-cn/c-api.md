...
ime_demo_capi 通过同一套 C ABI 提供实时终端界面，只把历史光标之前的前缀传给模型，并支持输入、Backspace/Delete 删除、左右移动光标、自动切分、候选编号提交、彩色生成延迟与 Ctrl-C 立即中止。没有活动拼音时，上下键可切换历史以测试 ContextManager 缓存复用；可选的第三个参数控制上下文数量，默认值为 2。
